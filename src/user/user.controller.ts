...
export class UserController {
  ...
  async createUser(
    ...
    try {
      ...
      let user = await this.userService.findUserDetails(createUserLoginDto);
      if (user) {
        ...
        // const userHash = await this.userService.getUserHash(
  ...
  async getUserDetailsByWalletAddress(
    ...
    try {
      const user: User = await this.userService.findUserByWalletAddress(
        walletAddress
      );
      if (!user) {
        return this.responseModel.response(
